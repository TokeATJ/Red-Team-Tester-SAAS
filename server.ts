...
async function runMultiAgentScanEngine(scanId: string, hostname: string, url: string | null, targetType: string) {
  ...
  try {
    ...
    if (ai) {
      try {
        const prompt = `You are the lead AI security analyst for RedMind AI. Perform a passive security review and STRIDE threat modeling report for:
Hostname: ${hostname}
...
app.get("/api/reports/download/:id/:type", (req, res) => {
  ...
  if (type === "executive") {
    filename = `RedMind_Executive_Report_${scan.id}.txt`;
    content = `================================================================================
REDMIND AI — EXECUTIVE RISK & GOVERNANCE REPORT
================================================================================
...
Generated by RedMind AI Autonomous Security Platform
================================================================================`;
  } else if (type === "technical") {
    filename = `RedMind_Technical_Report_${scan.id}.txt`;
    content = `================================================================================
...
================================================================================
REDMIND AI SECURE PIPELINE WORKERS
================================================================================`;
  } else {
    filename = `RedMind_Compliance_Audit_${scan.id}.txt`;
    content = `================================================================================
REDMIND AI — REGULATORY COMPLIANCE AUDIT MATRICES
================================================================================
Target: ${scan.target}
...
async function startServer() {
  ...
  app.listen(PORT, "0.0.0.0", () => {
    console.log(`[RedMind AI] Server running securely on http://localhost:${PORT}`);
  });
}
